...
const actionsElement = document.getElementsByClassName('search_actions')[0];
...
function createSuggestionElement({suggestion, auxiliaryData}) {
  ...
  return `<li class="search_suggestions_list_result">${boldProcessedSuggestion}${auxiliaryString}</li>`
}
...
function onSuggestionsResponse(data) {
  ...
  if (suggestionsHTML) {
    actionsElement.classList.add('search_actions-autosuggest');
    //actionsElement.classList.add("search-autosuggest"); doesnt do anything by the looks of it
  } else {
    actionsElement.classList.remove('search_actions-autosuggest');
    //actionsElement.classList.remove("search-autosuggest"); doesnt do anythinge either
  }
}
...
function onNewInput(event) {
  if (searchInput.value) {
    ...
    actionsElement.classList.remove('search_actions-autosuggest');
    actionsElement.classList.remove("search-autosuggest"); //Moves buttons back properly, without it the buttons keep the background if the list has popped up and gone away again
  }
}
